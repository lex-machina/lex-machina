...
export function usePreprocessing(): UsePreprocessingReturn {
    ...
    const loadLastResult =
        useCallback(async (): Promise<PreprocessingSummary | null> => {
            try {
     ...
     */
    const setDisplaySummary = useCallback(
        (newSummary: PreprocessingSummary) => {
    ...
    useEffect(() => {
        ...
        if (!hasLoadedRef.current && !isProcessingRef.current) {
            ...
            invoke<PreprocessingSummary | null>("get_last_preprocessing_result")
                ...
                .catch((err) => {
                    console.error(
                        "Failed to load last preprocessing result:",
                        err,
                    );
                });
        }
