...
export function useSettings(): UseSettingsReturn {
    ...
    const configureAIProvider = useCallback(
        async (provider: AIProviderType, apiKey: string) => {
            if (provider === "none") {
                throw new Error(
                    "Use clearAIProvider to remove AI configuration",
                );
            }
    ...
    const switchProvider = useCallback(async (provider: AIProviderType) => {
        try {
            ...
            const newConfig = await invoke<AIProviderConfig | null>(
     ...
     */
    const deleteSavedProvider = useCallback(
        async (provider: AIProviderType) => {
            try {
                ...
                setAIConfig((prev) =>
                    prev?.provider === provider ? null : prev,
                );
                setValidationStatus("idle");
                setValidationError(null);
            } catch (err) {
