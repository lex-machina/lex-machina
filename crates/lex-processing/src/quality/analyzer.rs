...
impl DataQualityAnalyzer {
    pub fn identify_issues(dataset_profile: &DatasetProfile, df: &DataFrame) -> Result<Vec<DataQualityIssue>> {
    ...
    fn analyze_outliers(profile: &DatasetProfile, df: &DataFrame) -> Result<Vec<DataQualityIssue>> {
        ...
        for col in &profile.column_profiles {
            if col.inferred_type == "numeric" 
                && col.characteristics.get("has_outliers")
                    ...
                    .unwrap_or(false) 
            {
                ...
                if !series.is_empty() {
                    ...
                    details.insert("normal_range".to_string(), 
                        serde_json::json!(format!("{:.2} to {:.2}", normal_min, normal_max)));
                    details.insert("outlier_examples".to_string(), 
                        serde_json::json!(outlier_examples));
                    details.insert("outlier_count".to_string(), 
        ...
        if !numeric_cols_with_outliers.is_empty() {
            ...
            for col_name in &numeric_cols_with_outliers {
                if let Some(details) = outlier_details.get(col_name) {
                    let normal_range: Option<&str> = details.get("normal_range")
                        .and_then(|v: &serde_json::Value| v.as_str());
                    let outlier_count: u64 = details.get("outlier_count")
                        .and_then(|v: &serde_json::Value| v.as_u64())
                        .unwrap_or(0);
                    let examples: String = details.get("outlier_examples")
            ...
            detection_details.insert("outlier_details".to_string(), serde_json::json!(outlier_details));
            ...
            issues.push(DataQualityIssue {
                ...
                business_impact: "Outliers may represent errors or important edge cases".to_string(),
                detection_details,
                suggested_solutions: vec![
                    SolutionOption {
                        option: "keep_outliers".to_string(),
                        description: "Keep outliers - they might represent important patterns".to_string(),
                        pros: Some("No information loss, captures full business reality".to_string()),
                    ...
                    SolutionOption {
                        ...
                        pros: Some("Reduces extreme influence while preserving patterns".to_string()),
    ...
    fn analyze_missing_values(profile: &DatasetProfile, df: &DataFrame) -> Result<Vec<DataQualityIssue>> {
        let mut issues = Vec::new();
            ...
            for col_profile in columns_with_missing {
                ...
                let actual_dtype = if let Ok(col) = df.column(&col_profile.name) {
                    match col.dtype() {
                        ...
                        DataType::Float32 | DataType::Float64 => "numeric",
                ...
                let mut detection_details = HashMap::new();
                detection_details.insert("column_name".to_string(), serde_json::json!(col_profile.name));
                detection_details.insert("actual_dtype".to_string(), serde_json::json!(actual_dtype));
                detection_details.insert("inferred_type".to_string(), serde_json::json!(col_profile.inferred_type));
                detection_details.insert("missing_count".to_string(), serde_json::json!(missing_count));
                detection_details.insert("missing_percentage".to_string(), serde_json::json!(missing_pct));
                detection_details.insert("total_rows".to_string(), serde_json::json!(total_rows));
                detection_details.insert("unique_values".to_string(), serde_json::json!(col_profile.unique_count));
                detection_details.insert("sample_values".to_string(), serde_json::json!(sample_values));
                ...
                let description = format!(
                    ...
                    total_rows,
                    actual_dtype,
                    if actual_dtype == "numeric" { "Integer or Float" } 
                    else if actual_dtype == "string" { "Text/Categorical" }
                    else if actual_dtype == "datetime" { "Date/Time" }
                    else { "Other" },
                    col_profile.unique_count,
                    sample_values
                );
        ...
        match actual_dtype {
            "numeric" => {
                ...
                strategies.push(SolutionOption {
                    ...
                    best_for: Some(format!("Numeric column with {} unique values", unique_count)),
                });
            ...
            "string" | "categorical" => {
                ...
                strategies.push(SolutionOption {
                    ...
                    pros: Some(format!("Fast, preserves distribution ({} categories)", unique_count)),
                ...
                if unique_count <= 10 {
                    strategies.push(SolutionOption {
                        ...
                        description: format!("Add 'Missing' as new category ({} existing)", unique_count),
...
mod tests {
    ...
    fn test_identify_issues_skips_identifier_columns() {
        let profile = create_profile(vec![create_column_profile(
            "id", "Int64", "numeric", 20, 20.0, 100, "identifier", false,
        )]);
        ...
        assert!(!issues.iter().any(|i| i.issue_type.contains("missing_values_id")));
    }
    ...
    fn test_analyze_outliers_no_outliers() {
        let profile = create_profile(vec![create_column_profile(
            "value", "Float64", "numeric", 0, 0.0, 10, "feature", false, // has_outliers = false
        )]);
