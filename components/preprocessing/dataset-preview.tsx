 ...
 */
function getDtypeCategory(dtype: string): "numeric" | "string" | "boolean" | "datetime" | "other" {
  const lower = dtype.toLowerCase();
  if (lower.includes("int") || lower.includes("float") || lower.includes("f64") || lower.includes("f32")) {
    return "numeric";
  }
  if (lower.includes("str") || lower.includes("utf8") || lower.includes("string")) {
    return "string";
  }
 ...
 */
function getCategoryColorClass(category: "numeric" | "string" | "boolean" | "datetime" | "other"): string {
  switch (category) {
    case "numeric":
      ...
      {categories.map((cat) => (
        <span
          ...
          className={cn(
            ...
            getCategoryColorClass(cat.key as "numeric" | "string" | "boolean" | "datetime" | "other")
          )}
        >
          {cat.count} {cat.label}
        </span>
...
function MissingValuesSummary({ columns, totalRows }: MissingValuesSummaryProps) {
  const stats = useMemo(() => {
    let totalMissing = 0;
    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="grid grid-cols-3 gap-4 text-xs">
        <div className="flex flex-col">
          ...
          <span className="font-medium tabular-nums">{formatNumber(stats.totalMissing)}</span>
        </div>
        <div className="flex flex-col">
          <span className="text-muted-foreground">Columns affected</span>
          <span className="font-medium tabular-nums">{stats.columnsWithMissing}</span>
        </div>
        <div className="flex flex-col">
          <span className="text-muted-foreground">Dataset coverage</span>
          <span className="font-medium tabular-nums">{stats.missingPercent.toFixed(1)}%</span>
        </div>
      </div>
    </div>
  );
  ...
  if (!fileInfo) {
    return (
      ...
      >
        ...
        <p className="text-xs mt-1">Load a CSV file to start preprocessing</p>
      </div>
    );
  }
    ...
    >
      ...
      <div className="flex items-start justify-between">
        <div className="flex flex-col gap-1 min-w-0">
          <h3 className="text-sm font-medium truncate" title={fileInfo.name}>
            {fileInfo.name}
          </h3>
          <p className="text-xs text-muted-foreground truncate" title={fileInfo.path}>
            {fileInfo.path}
          </p>
        </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-col gap-1 max-h-[120px] overflow-y-auto">
            {fileInfo.columns
              ...
              .slice(0, 5)
              .map((col) => {
                const percent = (col.null_count / fileInfo.row_count) * 100;
                return (
                  <div
                    key={col.name}
            ...
            {fileInfo.columns.filter((c) => c.null_count > 0).length > 5 && (
              <span className="text-xs text-muted-foreground">
                +{fileInfo.columns.filter((c) => c.null_count > 0).length - 5} more columns
              </span>
            )}
          </div>
        </div>
