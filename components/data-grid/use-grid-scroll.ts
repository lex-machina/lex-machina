...
export interface GridScrollConfig {
  ...
  onVisibleRangeChange?: (currentRowIndex: number, visibleRowCount: number) => void;
}
...
export function useGridScroll(config: GridScrollConfig): GridScrollState {
  ...
  const maxScrollLeft = Math.max(0, totalWidth - viewportWidth + SCROLLBAR_SIZE);
  ...
  const persistScrollPosition = useCallback((rowIndex: number, scrollLeftPos: number) => {
  ...
  const handleWheel = useCallback(
    (e: React.WheelEvent) => {
      ...
      if (e.deltaX !== 0) {
        ...
        const scrollDelta =
          Math.trunc(accumulatedHorizontalDelta.current / PIXELS_PER_HORIZONTAL_SCROLL) *
          PIXELS_PER_HORIZONTAL_SCROLL;

        if (scrollDelta !== 0) {
          accumulatedHorizontalDelta.current -= scrollDelta;
          setScrollLeft((prev) => clamp(prev + scrollDelta, 0, maxScrollLeft));
        }
      }
    },
    [clamp, maxScrollLeft]
